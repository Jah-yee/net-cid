...
- **Out-of-scope observation (corrected):** I first reported #16's depth/output-cap hardening as missing from `CHANGELOG.md`. That was wrong — the entries existed under an orphaned `## [Released]` heading (where #16 placed them), not folded into `## [1.6.0]` like its siblings #13/#14.

## Follow-up (PR #38 review)

- **Reviewer note — message parity.** The `JsonElement` path named the duplicate key (`'a'`) but the `JsonNode` path's translated message did not. Fix: the `JsonNode` pre-walk `catch (ArgumentException)` no longer translates; it falls through to the serialize + `WriteElement` path, so `WriteObject` (the single source of truth) emits the same key-naming message on both surfaces. `SerializeToDocument` preserves duplicates, so the offender is guaranteed to reach `WriteObject` — the path stays fail-closed. Updated `JsonNode_Overload_Duplicate_Keys_Throw_JcsFormatException` to assert the keyed message and added `Both_Overloads_Report_Identical_Keyed_Duplicate_Message` (asserts `ex.Message` equality across overloads). Trade-off: the original `ArgumentException` is no longer chained as `InnerException` (the key is now in the main message, so digging is unnecessary).
- **CHANGELOG consolidation (#16).** Moved #16's Added + Security bullets out of the orphaned `## [Released]` section into `## [1.6.0]`, alongside #13/#14/#17, and restored `## [Released]` to an empty placeholder. 1.6.0 now records all of its work in one place.
- Re-verified: build 0 warnings (`-warnaserror`); unit 208 passed / 1 skipped (9 duplicate/parity tests); integration 6 passed; example prints `duplicate key rejected: Duplicate object member name 'a' is not allowed …`. No adversarial pass (per request — these are simple, localized changes).
